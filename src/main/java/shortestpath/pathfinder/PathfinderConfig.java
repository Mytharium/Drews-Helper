...
public class PathfinderConfig
{
	...
	private Set<String> blockedTransportKeys = Set.of();
	@Getter
	...
	public void refresh()
	{
		...
		blockedTransportKeys = ShortestPathPlugin.overrideStringSet("blockedTransportKeys");
	...
	private boolean useTransport(Transport transport)
	{
		...
		if (isBlockedTransport(transport))
		{
			return false;
		}
	...
	private boolean isBlockedTransport(Transport transport)
	{
		...
		String key = transportKey(transport);
		if (key.isEmpty())
		{
			return false;
		}

		for (String blockedKey : blockedTransportKeys)
		{
			if (key.equals(blockedKey) || key.startsWith(blockedKey + "_"))
			{
				return true;
			}
		}
	...
	static String transportKey(Transport transport)
	{
		TransportType type = transport.getType();
		if (type == null || type.getResourcePath() == null)
		{
			return "";
		}

		String typeKey = slug(type.getResourcePath()
			.replace("/transports/", "")
			.replace(".tsv", ""));
		if (typeKey.isEmpty())
		{
		...
		String label = transport.getDisplayInfo() == null || transport.getDisplayInfo().trim().isEmpty()
			? transport.getObjectInfo()
			: transport.getDisplayInfo();
		String labelKey = slug(cleanTransportLabel(label));
		return labelKey.isEmpty() ? "" : typeKey + ":" + labelKey;
	}

	private static String cleanTransportLabel(String label)
	{
		if (label == null)
		{
			return "";
