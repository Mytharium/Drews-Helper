...
- `Routing Options` now includes `Route Solver` (`A*` default, `BFS` optional) and `Benchmark Movement` (default OFF). Benchmark mode solves both A* and BFS for the same waypoint route, keeps the selected solver as the displayed path, and logs `DREW_ROUTE_BENCH` movement comparisons while the player walks.
- Benchmark reports compare first-step direction, first 5 and 10 movement ticks, full tile-sequence equality, path-length delta, max lateral deviation, turn-count delta, solve time, and expanded-node count. They also log coordinate traces for route start, target, first 10 primary/alternate predicted path tiles, first 10 actual movement tiles, first divergence index, predicted/actual windows around the divergence, and legal candidate moves from the fork tile with predicted/actual choices marked. Use those logs to decide whether BFS should replace A* for walking routes, or whether the mismatch is really start/target/click-tile alignment versus movement-order tie behavior.
...
- Myth's post-`D-0040` Path 1 and Path 3 samples still diverge late. Temporary branch probes showed the client walked diagonal continuations that Drew's static collision graph ranks as longer than the live client path: southwest branch `(2939,3222,0) -> (2938,3221,0)` and northeast branch `(2967,3231,0) -> (2968,3230,0)`. Treat those as collision-map/live-client disagreement until proven otherwise, not more A* versus BFS evidence.
